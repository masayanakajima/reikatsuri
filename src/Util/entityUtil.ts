...
export namespace entityUtil {
	...
	export function createLabel(
		...
		_maxLength: number, _align: g.TextAlign,fontSize?:number): g.Label {
	...
	export function createNumLabel(
		_scene: g.Scene, _bitmapFont: g.BitmapFont, _digit: number,fontSize?:number): g.Label {
		const nums: string[] = [];
		...
		const label = createLabel(
			_scene, text, _bitmapFont, _digit, g.TextAlign.Right,fontSize);
		return label;
	}
	...
	export function moveNumLabelTo(
		...
		_label.x = _x + (<g.BitmapFont>_label.font).defaultGlyphWidth - _label.width;
		_label.y = _y;
		_label.modified();
	}
	...
	export function createWhiteLabel(
		_digit: number,
	...
	export function createRedLabel(
		_digit: number,
		_posStart: g.CommonOffset,
		_scene:g.Scene,
		fontSize?:number
